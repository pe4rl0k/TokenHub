...
const TokenCreationForm: React.FC = () => {
  ...
  const createTokenOnBlockchain = async () => {
    ...
    const tokenDecimals = parseInt(decimalCount, 10);
    const supplyAdjustedForDecimals = web3Instance.utils.toBN(initialSupply)
      .mul(web3Instance.utils.toBN(10).pow(web3Instance.utils.toBN(tokenDecimals)));
    
    try {
      setDeploymentStatus('Deploying token...');
      const estimatedGas = await tokenFactoryContract.methods
  ...
  return (
    <div>
      ...
      <input 
        type="text" 
        value={decimalCount} 
        onChange={(e) => setDecimalCount(e.target.value)} 
      />
      ...
      {resultingTokenAddress && (
        <div>
          ...
          <a 
            href={`https://etherscan.io/address/${resultingTokenAddress}`} 
            target="_blank" 
